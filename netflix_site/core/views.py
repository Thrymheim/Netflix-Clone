...
def signup(request):
    if request.method == 'POST':
        ...
        # ایمیل چک بشه نبود یوزر جدید بساز با تابع try
        ...
        try:
            ...
            #برای اینکه یوزر اتوماتیک لوگین بشه
            user_login = auth.authenticate(username=username, password=password)
            if user_login is not None:
                auth.login(request, user_login)
                return redirect('/')
            else:
                messages.info(request, 'Error logging in after signup')
                return redirect('login')

        except Exception as e:
            messages.info(request, f'Error creating account: {str(e)}')
            return redirect('signup')
